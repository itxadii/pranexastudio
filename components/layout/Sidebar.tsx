...
export const Sidebar: React.FC<SidebarProps> = ({ role, userName, userEmail }) => {
  ...
  const adminLinks = [
    { label: "Dashboard", href: "/admin", icon: (
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-5 h-5">
        <path strokeLinecap="round" strokeLinejoin="round" d="m2.25 12 8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21M7.5 9.75h9m-9 5.25h9" />
      </svg>
    )},
  ...
  const trainerLinks = [
    ...
    { label: "History Feed", href: "/trainer/history", icon: (
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-5 h-5">
  ...
  const customerLinks = [
    ...
    { label: "My Progress", href: "/customer/progress", icon: (
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-5 h-5">
  ...
  const links = role === "ADMIN" ? adminLinks : (role === "TRAINER" ? trainerLinks : customerLinks);
    ...
    <>
      ...
      <div className="lg:hidden h-16 border-b border-cream-dark bg-white/95 backdrop-blur px-6 flex items-center justify-between sticky top-0 z-30">
        ...
        <button 
          ...
          className="p-2 text-text-dark hover:text-deep-teal transition-colors"
      ...
      {/* Sidebar container */}
      <aside className={`
        fixed inset-y-0 left-0 z-40 w-64 border-r border-cream-dark bg-white flex flex-col justify-between p-6 transition-transform duration-300 lg:translate-x-0 lg:sticky lg:h-screen
      ...
      `}>
        <div className="space-y-8">
          ...
          <nav className="flex flex-col gap-1">
            {links.map((link) => {
              const active = pathname === link.href || (link.href !== "/admin" && link.href !== "/trainer" && link.href !== "/customer" && pathname.startsWith(link.href));
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  ...
                  className={`
                    ...
                    ${active 
                      ? "bg-deep-teal/5 text-deep-teal shadow-xs font-bold" 
                      : "text-text-muted hover:text-text-dark hover:bg-cream-dark/30"}
                  `}
                >
                  <span className={active ? "text-deep-teal" : "text-text-muted"}>
                    {link.icon}
                  </span>
                  {link.label}
                </Link>
        ...
        {/* User Block & Logout */}
        <div className="space-y-4 pt-6 border-t border-cream-dark/50">
          <div className="px-2">
            <h4 className="font-bold text-sm text-text-dark truncate">{userName}</h4>
            <p className="text-xs text-text-muted truncate">{userEmail}</p>
          </div>
          <button
            onClick={handleLogout}
            className="flex items-center gap-3 w-full px-4 py-3 rounded-lg text-[15px] font-semibold text-red-600 hover:bg-red-50 transition-colors"
          >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-5 h-5">
